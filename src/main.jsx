  ...
  return <div className={`apex-shell theme-${theme} surface-${surface} ${day?'day':'night'} profile-${displayProfile} ${reducedMotion?'reduce-motion':''}`} style={style}>
    {!commissioned?<Commissioning finish={finish} act={act}/>:<div className="calibrated-stage">
      ...
      <main className="apex-canvas">
        ...
        {active==='settings'&&<SettingsPage {...{theme,setTheme,surface,setSurface,accent,setAccent,iconScale,setIconScale,reducedMotion,setReducedMotion,displayMode,setDisplayMode,displayProfile,setDisplayProfile,calibration,setCalibration,visual,setVisual,preferences,setPreferences,device,viewport,act,notify}} obd={live.obd}/>}
      </main>
...
function SplitPicker({close,act,notify}){
  ...
  return <div className="modal-scrim" onClick={close}><section className="modal split-picker" role="dialog" aria-modal="true" aria-label="Split launcher with app" onClick={e=>e.stopPropagation()}><header><h2>Split with TRX APEX</h2><button aria-label="Close dialog" onClick={close}><X/></button></header><p>Choose an app to place beside the launcher. To pair two outside apps, open the first app and use Android Recents → Split screen, then select the second app.</p><div className="split-options">{apps.length?apps.map(app=><button key={app.packageName} onClick={async()=>{const r=await act(native.launchAdjacent(app.packageName));if(r?.success)notify(r.message||'Split screen requested.');close();}}>{app.icon?<img src={app.icon} alt=""/>:<Grid2X2/>}<span>{app.name}</span></button>):<p>No launchable apps found.</p>}</div></section></div>;
}
...
function AppsPage({act,notify,quick,openSplit}){
  ...
  return <section className={'page apps-page '+(expanded?'expanded':'')} aria-label="Apps page"><div className="search-box"><Search/><input aria-label="Search apps" placeholder="Search apps" value={query} onChange={e=>setQuery(e.target.value)}/>{query&&<button aria-label="Clear app search" onClick={()=>setQuery('')}><X/></button>}<button aria-label="Split launcher with app" onClick={openSplit}><PanelLeft/></button><button aria-label={editing?'Finish editing favorites':'Edit favorites'} className={editing?'active':''} onClick={()=>setEditing(!editing)}>{editing?<Check/>:<SlidersHorizontal/>}</button></div>
    ...
    {context&&<Modal title={context.name} close={()=>setContext(null)}><Action label="Split with TRX APEX" detail="Open this app beside the launcher" onClick={async()=>{const app=context;setContext(null);const r=await act(native.launchAdjacent(app.packageName));if(r?.success)notify(r.message||'Split screen requested.');}}/><Action label="Open for Android split" detail="Then use Android Recents → Split screen to choose the second app" onClick={()=>{const app=context;setContext(null);act(native.launchApp(app.packageName));}}/><Action label={favorites.includes(context.packageName)?'Remove favorite':'Add favorite'} onClick={()=>{toggle(context);setContext(null);}}/><Action label="App information" onClick={()=>{act(native.appAction(context.packageName,'info'));setContext(null);}}/><Action label="Uninstall app" detail="Android will ask you to confirm" onClick={()=>{act(native.appAction(context.packageName,'uninstall'));setContext(null);}}/></Modal>}
  </section>;
}
...
function SettingsPage(p){
  const {theme,setTheme,surface,setSurface,accent,setAccent,iconScale,setIconScale,reducedMotion,setReducedMotion,displayMode,setDisplayMode,displayProfile,setDisplayProfile,calibration,setCalibration,visual,setVisual,preferences,setPreferences,device,viewport,act,obd}=p;
