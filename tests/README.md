...
The UI suite covers 28 page/viewport combinations (including Weather), five themes, persistence, calibration reset, media command dispatch, unsupported Like, navigation search/route options, OBD reconnect dispatch, installed app launch, two-app split request dispatch, visualizer style changes, six favorites at maximum icon scale, and preserving intentionally empty favorites. Bounds checks cover page sections; screenshot review complements them. Browser tests cannot validate native map rendering, a player's acknowledgement, ECU communication, actual split-screen placement, or persistent Android overlays.
